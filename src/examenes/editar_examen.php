    ...
    try {
        if ($hasPrecioConvenio) {
            $stmt = $pdo->prepare("UPDATE examenes SET 
                ...
                preanalitica_cliente = ?, preanalitica_referencias = ?, tipo_muestra = ?, tipo_tubo = ?, observaciones = ?, precio_publico = ?, precio_convenio = ?, adicional = ?, vigente = ?
                WHERE id = ?");
            $stmt->execute([
                $codigo,
                $nombre,
        ...
        $_SESSION['mensaje'] .= " Los resultados históricos conservarán su snapshot actual hasta actualizar manualmente cada examen.";
