// Creador de select para tipo de fila
...
// Grupo de inputs para valores de referencia
...
// Función para agregar una fila al builder
...
// Delegación segura para eliminar fila
...
// Agregar nueva fila, solo si el botón existe
...
// Delegación segura para agregar valor de referencia
document.addEventListener('click', function(e) {
  if (e.target.classList && e.target.classList.contains('add-ref')) {
    const refList = e.target.parentElement.querySelector('.valores-ref-list');
    if (refList && typeof createRefGroup === "function") {
...
// Delegación segura para eliminar valor de referencia
document.addEventListener('click', function(e) {
  if (e.target.classList && e.target.classList.contains('remove-ref')) {
    if (e.target.parentElement) e.target.parentElement.remove();
    if (typeof updatePreview === "function") updatePreview();
  }
});

// Actualizar vista previa en tiempo real, solo si la tabla existe
var formatTable = document.getElementById('formatTable');
if (formatTable) {
  formatTable.addEventListener('input', function() {
    if (typeof updatePreview === "function") updatePreview();
  });
  formatTable.addEventListener('change', function() {
    if (typeof updatePreview === "function") updatePreview();
  });
}
function updatePreview() {
  var tbody = document.querySelector('#formatTable tbody');
  if (!tbody) return;
...
var formExamen = document.getElementById('form-examen');
if (formExamen) {
  formExamen.addEventListener('submit', function(e) {
    // ... tu lógica ...
    var adicionalInput = document.getElementById('adicional');
    if (adicionalInput) {
      adicionalInput.value = JSON.stringify(formato);
      // PEGA AQUÍ EL CONSOLE.LOG
      console.log('Enviando adicional:', adicionalInput.value);
    }
  });
}
